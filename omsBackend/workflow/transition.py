...
class TransitionExecutor(object):
    def __init__(self, operator, instance, task, transition=None, desc=''):
    ...
    def execute(self):
        ...
        need_transfer = False
        if self.transition.routing_rule == 'joint' and self.transition.code not in ['back', 'rollback']:
    ...
    def _complete_task(self, need_transfer):
        ...
        if not event:
            event = create_event(
                instance, transition,
                desc=self.desc, user=self.operator,
    ...
    def _send_notification(self):
        ...
        #     pk__in=[self.operator.pk, instance.create_time.pk]).distinct()

        # wf_send_msg(notice_users, 'notify', last_event)
        # if instance.create_time != self.operator:
        #     wf_send_msg([instance.create_time], 'transfered', last_event)
    ...
    def _gen_new_task(self):
        ...
        #
        #     # notify next operator(not include current operator and instance.create_time)
        #     if operator not in [self.operator, self.instance.create_time]:
        #         need_notify_operators.append(operator)
        #
        #     agent_user = new_task.agent_user
        #     if agent_user and agent_user not in [self.operator, self.instance.create_time]:
    ...
    def update_users_on_transfer(self):
        ...
        # next_operators = to_node.get_operators(instance.create_time, self.operator, instance)
        # event.next_operators.add(*next_operators)
        # notice_users = to_node.get_notice_users(instance.create_time, self.operator, instance)
        # event.notice_users.add(*notice_users)
        # can_view_users = to_node.get_share_users(instance.create_time, self.operator, instance)
