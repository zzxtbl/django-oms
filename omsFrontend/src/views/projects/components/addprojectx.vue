...
export default {
  ...
  created() {
    if (this.demand) {
      this.ruleForm.demand = this.demand.pid
    }
    this.getUsers()
    this.getTypes()
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          postProject(this.ruleForm).then(response => {
            ...
            const messageForm = {
              action_user: this.ruleForm.action_user.join() + ',' + this.ruleForm.follow_user.join() + ',' + response.data.test_user,
              title: `【新任务】${this.ruleForm.name}`,
